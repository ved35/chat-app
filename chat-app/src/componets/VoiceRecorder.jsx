...
function VoiceRecorder() {
  ...
  const { audio } = useSelector((state) => state.app.modal);

  useEffect(() => {
    const keyHandler = ({ keyCode }) => {
      if (!audio || keyCode !== 27) return;

      dispatch(updateAudioModal(false));
    };
      ...
      >
        ...
        >
          ...
          <div className="flex flex-row items-center space-x-4 w-full mt-8">
            ...
            <button className="w-full bg-transparent border border-red p-2 text-red" onClick={()=>{
                dispatch(updateAudioModal(false))
            }}>
              Cancel
            </button>
